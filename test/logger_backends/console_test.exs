defmodule LoggerBackends.ConsoleTest do
  use Logger.Case
  ...
  test "does not start when there is no user" do
    :ok = LoggerBackends.remove(LoggerBackends.Console)
  ...
  after
    {:ok, _} = LoggerBackends.add(LoggerBackends.Console)
  end
  ...
  test "configures format" do
    LoggerBackends.configure(LoggerBackends.Console, format: "$message [$level]")

    assert capture_log(fn -> Logger.debug("hello") end) =~ "hello [debug]"
  end

  test "configures metadata" do
    LoggerBackends.configure(LoggerBackends.Console,
  ...
  test "ignores crash_reason metadata when configured with metadata: :all" do
    LoggerBackends.configure(LoggerBackends.Console, format: "$metadata$message", metadata: :all)
    Logger.metadata(crash_reason: {%RuntimeError{message: "oops"}, []})
    assert capture_log(fn -> Logger.debug("hello") end) =~ "hello"
  end

  test "configures formatter to {module, function} tuple" do
    LoggerBackends.configure(LoggerBackends.Console, format: {__MODULE__, :format})
  ...
  test "configures metadata to :all" do
    LoggerBackends.configure(LoggerBackends.Console, format: "$metadata", metadata: :all)
    Logger.metadata(user_id: 11)
  ...
  test "provides metadata defaults" do
    ...
    LoggerBackends.configure(LoggerBackends.Console, format: "$metadata", metadata: metadata)
  ...
  test "configures level" do
    LoggerBackends.configure(LoggerBackends.Console, level: :info)

    assert capture_log(fn -> Logger.debug("hello") end) == ""
  end

  test "filter by notice" do
    LoggerBackends.configure(LoggerBackends.Console, level: :notice)
  ...
  test "configures colors" do
    LoggerBackends.configure(LoggerBackends.Console, format: "$message", colors: [enabled: true])

    assert capture_log(fn -> Logger.debug("hello") end) ==
             IO.ANSI.cyan() <> "hello" <> IO.ANSI.reset()

    LoggerBackends.configure(LoggerBackends.Console, colors: [debug: :magenta])
    ...
    LoggerBackends.configure(LoggerBackends.Console, colors: [info: :cyan])
    ...
    LoggerBackends.configure(LoggerBackends.Console, colors: [warning: :magenta])
    ...
    LoggerBackends.configure(LoggerBackends.Console, colors: [error: :cyan])
