defmodule LoggerBackends.Watcher do
  ...
  def init({handler, args}) do
    ...
    case :gen_event.delete_handler(LoggerBackends, handler, :ok) do
      {:error, :module_not_found} ->
        case :gen_event.add_sup_handler(LoggerBackends, handler, args) do
          :ok ->
            {:ok, handler}
  ...
  defp logger_has_backends?() do
    try do
      :gen_event.which_handlers(LoggerBackends) != [LoggerBackends.Config]
    catch
      _, _ -> false
    end
  end
  ...
  def terminate(_reason, handler) do
    ...
    :gen_event.delete_handler(LoggerBackends, handler, :ok)
    :ok
  end
